...
const BASE_PATH = '/freshdash-pro';

const STATIC_ASSETS = [
  BASE_PATH + '/',
  BASE_PATH + '/freshdash-pro-PWA.html',
  BASE_PATH + '',
  BASE_PATH + '',
    ...
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        if (event.request.destination === 'document') {
          return caches.match(BASE_PATH + '/freshdash-pro-PWA.html');
        }
      });
    })
  );
...
self.addEventListener('push', event => {
  let data = { title: 'توصيل فرش', body: 'لديك إشعار جديد', icon: BASE_PATH + '/icon-192.png' };
